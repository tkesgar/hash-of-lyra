...
describe('verify', () => {
  ...
  for (const hashFile of ['bun-hash.txt', 'phc-hash.txt', 'zig-hash.txt']) {
    ...
    fs.readFileSync(hashFilePath, 'utf-8').split('\n')
      .map(ln => ln.trim())
      .filter(Boolean)
      .forEach(ln => {
        const [password, hash] = ln.split(':')
        phcStrings.push({ password, hash })
      })
  }

  test.for(phcStrings)('should return true for verify($hash, $password)', async (tc, { expect, skip }) => {
    if (tc.slow && !import.meta.env.ALLOW_SLOW_TESTS) {
      skip('slow')
    }

    expect(await verify(tc.hash, tc.password, { scryptMaxmem: tc.maxmem })).toBe(true)
  })
})
